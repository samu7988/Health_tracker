...
#define EXTCOM_MIN_PORT       (gpioPortD)
#define EXTCOM_MIN_PIN        (13)


#define LCD_SENSOR_EN_PORT       (gpioPortD)
#define LCD_SENSOR_EN_PIN        (15)
...
void displayInit()
{
    ...
    turn_on_power(gpioPortD, 15); //LCD is connected on gpio Port D , pin 15
    ...
    //           set up a 1 second repeating soft timer and uncomment the following lines
	  ...
	  timer_response = sl_bt_system_set_soft_timer(ONE_SECOND, 0 ,false);
	  if (timer_response != SL_STATUS_OK) {
	      LOG_INFO("SOFT TIMER FAILED");
     }
